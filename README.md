...
#### Observability (dooers-service-otel)

**Creator contract:** install the extras — that is the only required step in agent code. `AgentServer` initializes OTEL and auto-instruments OpenAI / Anthropic / OpenAI Agents. No OpenTelemetry imports, no manual spans, no token accounting in the handler.

```bash
pip install "dooers-agents-server[observability]"
# or: uv add "dooers-agents-server[observability]"
```

Tracing uses Dooers platform defaults automatically (same idea as the baked analytics webhook URL). Creators do not need to set Core/otel URLs. Ensure `AGENT_SEED_SECRET` / core `settings.seed` so the runtime API key is available for export auth.
