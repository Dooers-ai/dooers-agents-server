...
- **`AgentConfig.whatsapp_peer_message`** (`ignore` | `register` | `dispatch`, default `register`) for WhatsApp human/`fromMe` peer messages on third-party chats.
- **`AgentServer.ingest(...)`** — persist a channel message on a thread without running the handler (used by `register`).

## [0.20.1] — 2026-08-31

### Added

- **`input_type="date"`** on `send.form_text` / wire `text_input` — native date picker contract; values and defaults use `YYYY-MM-DD`.
...
- **OTEL export for agents-first / 1:1 chat.** Mint tokens via `POST /identity/service-token/agent`, cache per agent+workspace, and export personal turns (empty `workspace_id`) with an org-scoped write token.
...
- **WebSocket heartbeat** — `ping` / `pong` frames so clients can keep long-lived connections alive through proxies and load balancers.

## [0.11.0] — 2026-06-17

### Changed

- **BREAKING — package rename.** The PyPI distribution is renamed **`dooers-agents-server` → `dooers-agents`** and the importable module is renamed **`dooers` → `dooers_agents`**. Update imports: `from dooers import AgentServer` → `from dooers_agents import AgentServer`. The public API is otherwise unchanged. Previously published versions remain available on PyPI under the old `dooers-agents-server` name; new releases publish as `dooers-agents`.
