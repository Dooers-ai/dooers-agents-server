...
def test_promote_skips_when_storage_none() -> None:
    ...
    with patch("dooers.agents.server.storage.chat_artifacts.gcs.download_bytes") as dl:
        promote_orphan_chat_artifact_if_present(
            cfg,
            agent_id="a1",
            thread_id="t1",
...
def test_promote_skips_when_no_orphan_bytes() -> None:
    cfg = _cfg_gcp()
    with (
        patch("dooers.agents.server.storage.chat_artifacts._gcs_blob_exists", side_effect=[True, False]),
        patch("dooers.agents.server.storage.chat_artifacts.gcs.copy_blob_same_bucket", return_value=None),
        patch("dooers.agents.server.storage.chat_artifacts.gcs.download_bytes", return_value=None) as dl,
    ):
        promote_orphan_chat_artifact_if_present(
            cfg,
            agent_id="a1",
...
def test_promote_gcp_uploads_then_deletes_source() -> None:
    ...
    with (
        patch("dooers.agents.server.storage.chat_artifacts._gcs_blob_exists", side_effect=[True, False]) as ex,
        patch("dooers.agents.server.storage.chat_artifacts.gcs.copy_blob_same_bucket", return_value=None),
        patch("dooers.agents.server.storage.chat_artifacts.gcs.download_bytes", return_value=payload) as dl,
        patch(
            "dooers.agents.server.storage.chat_artifacts.gcs.upload_bytes_to_blob_name",
            return_value="gs://test-bucket/x",
        ) as up,
        patch(
            "dooers.agents.server.storage.chat_artifacts._gcs_delete_blob_with_retries",
            return_value=True,
        ) as rm,
    ):
        promote_orphan_chat_artifact_if_present(
...
def test_promote_gcp_when_dest_exists_only_deletes_source() -> None:
    cfg = _cfg_gcp()
    with (
        patch("dooers.agents.server.storage.chat_artifacts.gcs.download_bytes") as dl,
        patch("dooers.agents.server.storage.chat_artifacts._gcs_blob_exists", side_effect=[True, True]),
        patch("dooers.agents.server.storage.chat_artifacts.gcs.upload_bytes_to_blob_name") as up,
        patch(
            "dooers.agents.server.storage.chat_artifacts._gcs_delete_blob_with_retries",
            return_value=True,
        ) as rm,
...
def test_promote_gcp_no_delete_when_upload_fails() -> None:
    ...
    with (
        patch("dooers.agents.server.storage.chat_artifacts._gcs_blob_exists", side_effect=[True, False]),
        patch("dooers.agents.server.storage.chat_artifacts.gcs.copy_blob_same_bucket", return_value=None),
        patch("dooers.agents.server.storage.chat_artifacts.gcs.download_bytes", return_value=b"data"),
        patch("dooers.agents.server.storage.chat_artifacts.gcs.upload_bytes_to_blob_name", return_value=None),
        patch(
            "dooers.agents.server.storage.chat_artifacts._gcs_delete_blob_with_retries",
            return_value=True,
        ) as rm,
...
def test_promote_gcp_server_side_copy_skips_download_upload() -> None:
    ...
    with (
        patch("dooers.agents.server.storage.chat_artifacts._gcs_blob_exists", side_effect=[True, False]),
        patch(
            "dooers.agents.server.storage.chat_artifacts.gcs.copy_blob_same_bucket",
